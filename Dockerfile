...
RUN set -x && chmod +x /usr/local/bin/kubectl \
    ...
    && cd /usr/local/src && wget ftp://ftp.vim.org/pub/vim/unix/vim-8.0.tar.bz2 && tar -xjf vim-8.0.tar.bz2 && cd vim80 && ./configure --prefix=/usr --with-features=huge --enable-pythoninterp --enable-luainterp && make && make install && cd .. && rm -rf vim80* \
    && wget https://github.com/downloads/libevent/libevent/libevent-2.0.21-stable.tar.gz && tar -xvzf libevent-2.0.21-stable.tar.gz && cd libevent-2.0.21-stable && ./configure --prefix=/usr/local && make && make install && cd .. && rm -rf libevent* \
    && wget https://github.com/tmux/tmux/releases/download/2.4/tmux-2.4.tar.gz && tar -xvzf tmux-2.4.tar.gz && cd tmux-2.4 && LDFLAGS="-L/usr/local/lib -Wl,-rpath=/usr/local/lib" ./configure --prefix=/usr/local && make && make install && cd .. && rm -rf tmux-2* \
...
RUN echo "export PATH=/opt/gerbil/bin:$PATH" >> ~/.zshrc_local
